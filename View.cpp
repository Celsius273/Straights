...
void View::updateCardsInHand(){
	...
	if(game_->gameState() == PLAYING){
        ...
        for(int j = 0; j < discardPile.size();j++){
			cout << "discardPile.RANKat(): " << discardPile.at(j) << endl;
            cardsInDiscard[j].set( deck.getCardImage(Card(Suit(discardPile.at(j)/13),Rank(discardPile.at(j)%13)) ) );
        }
    }
    if(game_->gameState() == ENDROUND || game_->gameState() == ENDGAME){
        //RESET PLAYEDCARD
...
void View::update(Notification n){
    ...
    switch (n){
        ...
        case WINNER:{
			Gtk::Dialog dialog( "Game over, winner found", *this );
			Gtk::Label winnerLabel( "Game is over, winner found" );
			Gtk::VBox* winnerContent = dialog.get_vbox();
			winnerContent->pack_start(winnerLabel, true, false);
			winnerLabel.show();
